...
const NewPlace = () => {
  ...
  async function uploadImage(file) {
    ...
    const respData = await fetch(
      "https://api.cloudinary.com/v1_1/yourhappyplaces/image/upload",
      {
        method: "post",
        body: data,
      }
  ...
  async function getCoordinates(address) {
   ...
   return returnCoordinates(results);
  }

  function returnCoordinates(results) {
    const location = {
  ...
  const placeSubmitHandler = async (event) => {
    ...
    try {
      ...
      formData.append( "location", await getCoordinates(formState.inputs.address.value)      );
